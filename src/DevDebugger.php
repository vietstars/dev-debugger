...
use Vietstars\DevDebugger\DataCollector\AuthCollector;
use Vietstars\DevDebugger\DataCollector\CacheCollector;
use Vietstars\DevDebugger\DataCollector\EventCollector;
use Vietstars\DevDebugger\DataCollector\FilesCollector;
use Vietstars\DevDebugger\DataCollector\GateCollector;
use Vietstars\DevDebugger\DataCollector\LaravelCollector;
use Vietstars\DevDebugger\DataCollector\LogsCollector;
use Vietstars\DevDebugger\DataCollector\ModelsCollector;
use Vietstars\DevDebugger\DataCollector\MultiAuthCollector;
use Vietstars\DevDebugger\DataCollector\QueryCollector;
use Vietstars\DevDebugger\DataCollector\SessionCollector;
use Vietstars\DevDebugger\DataCollector\RequestCollector;
use Vietstars\DevDebugger\DataCollector\ViewCollector;
use Vietstars\DevDebugger\Storage\SocketStorage;
use Vietstars\DevDebugger\Storage\FilesystemStorage;
...
use Vietstars\DevDebugger\DataCollector\PhpInfoCollector;
use DebugBar\DataCollector\RequestDataCollector;
use DebugBar\DataCollector\TimeDataCollector;
use Vietstars\DevDebugger\DataFormatter\QueryFormatter;
use Vietstars\DevDebugger\Support\Clockwork\ClockworkCollector;
 ...
 */
class DevDebugger extends DebugBar
{
    /**
    ...
    public function boot()
    {
        ...
        if (!$this->isLumen() && $this->shouldCollect('log', true)) {
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add LogsCollector to Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
        ...
        if ($this->shouldCollect('db', true) && isset($this->app['db'])) {
            ...
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add listen to Queries for Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
            ...
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add listen transactions to Queries for Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
        ...
        if ($this->shouldCollect('models', true)) {
            try {
                $modelsCollector = $this->app->make('Vietstars\DevDebugger\DataCollector\ModelsCollector');
        ...
        if ($this->shouldCollect('livewire', true) && $this->app->bound('livewire')) {
            try {
                $livewireCollector = $this->app->make('Vietstars\DevDebugger\DataCollector\LivewireCollector');
        ...
        if ($this->shouldCollect('auth', false)) {
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add AuthCollector to Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
        ...
        if ($this->shouldCollect('gate', false)) {
            try {
                $gateCollector = $this->app->make('Vietstars\DevDebugger\DataCollector\GateCollector');
        ...
        if ($this->shouldCollect('cache', false) && isset($this->app['events'])) {
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add CacheCollector to Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
    ...
    public function modifyResponse(Request $request, Response $response)
    {
        ...
        if ($this->shouldCollect('config', false)) {
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add ConfigCollector to Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
        ...
        if ($this->app->bound(SessionManager::class)) {
            ...
            if ($this->shouldCollect('session') && ! $this->hasCollector('session')) {
                try {
                    ...
                    $this->addThrowable(
                        new Exception(
                            'Cannot add SessionCollector to Dev Debugger: ' . $e->getMessage(),
                            $e->getCode(),
                            $e
                        )
                    );
        ...
        if ($this->shouldCollect('symfony_request', true) && !$this->hasCollector('request')) {
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add SymfonyRequestCollector to Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
        ...
        if ($app['config']->get('debugbar.clockwork') && ! $this->hasCollector('clockwork')) {
            try {
                ...
                $this->addThrowable(
                    new Exception(
                        'Cannot add ClockworkCollector to Dev Debugger: ' . $e->getMessage(),
                        $e->getCode(),
                        $e
                    )
                );
